...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   BUGS & "TO-FIX"'s
%   - Capire come incastrare Huffman con l'LZ77
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
symbol_probabilities = zeros(1, M);
...
while ~end_of_file
    ...
    if m == 0   % last symbol of the sequence: encode it with a single symbol (so doing I'm not wasting resources)
        ...
        last_symb = double(seq(end));
        dictionary(dict_index, :) = [0, 0, last_symb];
        dict_index = dict_index + 1;
        symbol_probabilities(last_symb - 1) = symbol_probabilities(last_symb - 1) + 1;
    else   
        conti = true;
        ...
        % New row in the dictionary
        last_symb = double(seq(coding_index + longest_match));
        symbol_probabilities(last_symb - 1) = symbol_probabilities(last_symb - 1) + 1;
        dictionary(dict_index, :) = [offset, longest_match, last_symb];
        dict_index = dict_index + 1;
    end
...
symbol_probabilities = symbol_probabilities / sum(symbol_probabilities);    % vector of the estimated probabilities

symbols = 0 : M - 1;    % symbols alphabet
[dict, avglen] = huffmandict(symbols, symbol_probabilities);    % Create dictionary.
